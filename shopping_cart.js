...
// Do not use the following two methods;  they are private to this class
    ...
    this.writeCookie = function() {  // PRIVATE METHOD
        ...
        toWrite += this.skuArray[i] + "|" + this.qtyArray[i] + "|" + this.costArray[i] +"||";
...
//////////////////////////////////////////////////////////////////////////            
    ...
    this.add = function(sku, quantity,cost) {
        sku = $.trim(sku);
        ...
        if(this.skuArray[i] == sku) {        
			...
			this.costArray[i] = parseFloat(cost,10) + parseFloat(this.costArray[i],10);
            found = true;            
            }
        if(!found) {       
    ...
    this.setQuantity = function(sku, quantity,cost) {  
        sku = $.trim(sku);
        var found = false;
        if(sku == "") return;        
        quantity = $.trim(quantity);  
        ...
        for(i=0; i < this.skuArray.length; i++)
            if(this.skuArray[i] == sku) {        
                ...
                this.costArray[i] = parseFloat(cost,10); 				
                found = true;
                }
        if(found)
            this.writeCookie();
